...
class OnlineExamController extends Controller
{
    ...
    public function teacherMarkAnswer(ManualMarkAnswerRequest $request, OnlineExamAnswer $answer)
    {
        ...
        $this->assertSubmissionMarkable($answer->submission);
    ...
    {
        $submission->loadMissing('exam');
        abort_unless((int) $submission->school_id === (int) $this->school_id, 404);
        $this->authorize('grade', $submission);
        abort_if($submission->status !== OnlineExamSubmission::STATUS_FINALIZED, 422, 'Only completely marked results can be released.');
        abort_if(($submission->exam->result_release_policy ?? 'immediate') !== 'manual', 422, 'This exam does not use manual result release.');

        $submission->update(['status' => OnlineExamSubmission::STATUS_RESULT_PUBLISHED]);
        \App\Support\OnlineExams\OnlineExamResultNotifier::resultAvailable($submission->fresh());
    ...
    private function submitBySubmission(OnlineExamSubmission $submission, string $submittedVia)
    {
        ...
        $locked = DB::transaction(function () use ($submission, $submittedVia) {
            ...
            $objectiveScore = 0.0;
            $manualScore = 0.0;
            $requiresManual = $locked->exam->questions()->whereNotIn('type', ['mcq', 'multiple_choice', 'true_false'])->exists();
    ...
    private function finalizeSubmission(OnlineExamSubmission $submission, string $via): void
    {
        ...
        DB::transaction(function () use ($submission, $via) {
            $locked = OnlineExamSubmission::whereKey($submission->id)->lockForUpdate()->with(['exam', 'answerRows.question'])->firstOrFail();
            $this->assertSubmissionMarkable($locked);
            $this->recomputeSubmissionScore($locked);
            if ($locked->status === OnlineExamSubmission::STATUS_PENDING_MANUAL) {
                abort(422, 'Finalize is blocked until all written answers are marked.');
            }
            $locked->update(['status' => OnlineExamSubmission::STATUS_FINALIZED, 'submitted_via' => $locked->submitted_via ?: $via]);
        });
    }
