    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
        <div class="d-flex flex-column">
            ...
            <ul class="d-flex align-items-center eBreadcrumb-2">
                <li><a href="{{ route('teacher.online_exams.index') }}">{{ get_phrase('Online Exams') }}</a></li>
                <li><a href="{{ route('teacher.online_exams.attempts', $exam->id) }}">{{ $exam->title }}</a></li>
                <li><a href="#">{{ get_phrase('Proctoring') }}</a></li>
            </ul>
        </div>
        <a class="export_btn bg-secondary" href="{{ route('teacher.online_exams.attempts', $exam->id) }}">{{ get_phrase('Back to Attempts') }}</a>
    </div>
</div></div></div>
