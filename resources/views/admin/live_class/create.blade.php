    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
                ...
                <a href="{{ route($routePrefix . '.live_classes.index') }}" class="eBtn eBtn-dark">{{ get_phrase('Back') }}</a>
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-12">
        <div class="eSection-wrap">
            <form method="POST" action="{{ route($routePrefix . '.live_classes.store') }}">
                @csrf
                @include('admin.live_class._form')
                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="eBtn eBtn-primary">{{ get_phrase('Save') }}</button>
                    <a href="{{ route($routePrefix . '.live_classes.index') }}" class="eBtn eBtn-dark">{{ get_phrase('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
