...
<div class="row justify-content-center"><div class="col-md-7"><div class="eSection-wrap">
    ...
    <div class="text-center py-4">
        <div class="display-1 {{ $existing->status=='graduated'?'text-success':($existing->status=='approved'?'text-primary':'text-warning') }}">
            <i class="bi bi-mortarboard-fill"></i>
        </div>
        <h4 class="mt-3">{{ get_phrase('Your Application Status') }}</h4>
        <span class="badge bg-{{ $existing->status=='graduated'?'success':($existing->status=='approved'?'primary':($existing->status=='pending'?'warning':'danger')) }} fs-6 mt-2">{{ ucfirst($existing->status) }}</span>
        @if($existing->classification)<div class="mt-2"><strong>{{ get_phrase('Classification') }}:</strong> {{ $existing->classification }}</div>@endif
        @if($existing->gpa)<div><strong>{{ get_phrase('GPA') }}:</strong> {{ $existing->gpa }}</div>@endif
    </div>
    @else
