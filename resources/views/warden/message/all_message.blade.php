...
@section('content')
    ...
    $msg_user_lists = DB::table('users')
        ->join('message_thrades', function ($join) {
            $join->on('users.id', '=', 'message_thrades.sender_id')->orWhere(function ($query) {
        ...
        ->where(function ($query) {
            $query->where('message_thrades.sender_id', auth()->user()->id)->orWhere('message_thrades.reciver_id', auth()->user()->id);
        })
                        ...
                        <div class="chat_users">
                            @foreach ($msg_user_lists as $msg_user_list)
                                @if ($msg_user_list->sender_id == auth()->user()->id || $msg_user_list->reciver_id == auth()->user()->id)
                                    <a href="{{ route('warden.message.all_message', ['id' => $msg_user_list->id]) }}" class="list-group-item list-group-item-action">
                                        <?php
                                        $unreadCount = DB::table('chats')->where('message_thrade', $msg_user_list->id)->where('read_status', 0)->count();
                        ...
                        <div class="chat_body">
                            ...
                            <div class="position-relative">
                                <div class="chat-messages p-4" id="chat_messages_body">
                                    ...
                                    @endforeach
                                    <?php
                                    $unreadCount = DB::table('chats')->where('message_thrade', $msg_user_details->thread_id)->where('read_status', 0)->count();
                            ...
                            <div class="flex-grow-0 py-3 px-4 border-top">
                                <div class="chat_write_box">
                                    <form action="{{ route('warden.message.chat_save') }}" method="post">
                                        @csrf
        ...
        $(document).ready(function() {
            $('#search').on('keyup', function() {
                ...
                if (query.trim() === '') {
                    ...
                    $.ajax({
                        url: "{{ route('warden.message.all_message', ['id' => $msg_user_details->thread_id]) }}",
                        type: "GET",
                        data: {
                            query: query
                        },
