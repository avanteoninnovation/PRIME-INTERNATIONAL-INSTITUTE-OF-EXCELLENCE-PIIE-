...
class OnlineExamBatch2DCalendarTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Schema::create('noticeboard', function (Blueprint $table) {
            $table->id(); $table->text('notice_title'); $table->text('notice'); $table->string('start_date');
            $table->string('start_time'); $table->string('end_date'); $table->string('end_time');
            $table->integer('status'); $table->integer('show_on_website'); $table->string('image');
            $table->integer('school_id'); $table->integer('session_id'); $table->timestamps();
        });
